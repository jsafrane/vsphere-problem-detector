...
func getPolicyDatastores(pctx context.Context, profileID types.PbmProfileId, vmClient *vim25.Client) ([]string, error) {
	...
	ctx, cancel = context.WithTimeout(pctx, *Timeout)
	defer cancel()
	v, err := m.CreateContainerView(ctx, vmClient.ServiceContent.RootFolder, kind, true)
	if err != nil {
		return nil, err
	}
