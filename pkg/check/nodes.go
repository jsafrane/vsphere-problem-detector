...
func checkDiskUUID(pctx context.Context, node *v1.Node, vmClient *vim25.Client, config *vsphere.VSphereConfig) error {
	vm, err := getVM(pctx, node, vmClient, config)
	if err != nil {
		return err
	}
...
func getVM(pctx context.Context, node *v1.Node, vmClient *vim25.Client, config *vsphere.VSphereConfig) (*object.VirtualMachine, error) {
	ctx, cancel := context.WithTimeout(pctx, *Timeout)
	defer cancel()
